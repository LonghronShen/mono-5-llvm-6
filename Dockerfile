...
RUN cp /etc/apt/sources.list /etc/apt/sources.list.bak && \
	...
	echo deb http://apt.llvm.org/jessie/ llvm-toolchain-jessie-6.0 main > /etc/apt/sources.list.d/llvm.list && \
	echo deb-src http://apt.llvm.org/jessie/ llvm-toolchain-jessie-6.0 main >> /etc/apt/sources.list.d/llvm.list && \
	apt-get update && apt-get install -y llvm-6.0 clang-6.0 lldb-6.0 liblldb-6.0-dev libunwind8 libunwind8-dev gettext libicu-dev liblttng-ust-dev libcurl4-openssl-dev libssl-dev libkrb5-dev libnuma-dev && \
